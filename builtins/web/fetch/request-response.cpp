...
<<<<<<< Updated upstream
    // redirect: "follow" - fall through to default behavior (the WASI HTTP
    // layer already followed the redirect).
=======
    ...
    auto *new_request = host_api::HttpOutgoingRequest::make(
        std::string_view{new_method}, std::move(new_url), std::move(new_headers));
