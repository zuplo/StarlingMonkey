...
Request::RedirectMode Request::redirect_mode(JSObject *obj) {
  ...
  return static_cast<RedirectMode>(
      JS::GetReservedSlot(obj, std::to_underlying(Slots::Redirect)).toInt32());
}
...
bool Request::clone(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  SetReservedSlot(new_request, std::to_underlying(Slots::Redirect),
                  GetReservedSlot(self, std::to_underlying(Slots::Redirect)));
...
void Request::init_slots(JSObject *requestInstance) {
  ...
  JS::SetReservedSlot(requestInstance, std::to_underlying(Slots::Redirect),
...
bool Request::initialize(JSContext *cx, JS::HandleObject request, JS::HandleValue input,
  ...
  if (is_instance(input)) {
    ...
    JS::SetReservedSlot(request, std::to_underlying(Slots::Redirect),
                        JS::GetReservedSlot(input_request, std::to_underlying(Slots::Redirect)));
...
bool ResponseFutureTask::run(api::Engine *engine) {
  ...
  // Check redirect mode and handle redirect responses accordingly.
  uint16_t status = Response::status(response_obj);
  ...
  if (is_redirect) {
    ...
    if (redirect_mode == Request::RedirectMode::Error) {
      // redirect: "error" - reject with a TypeError.
      api::throw_error(cx, FetchErrors::FetchRedirectError);
      return RejectPromiseWithPendingError(cx, response_promise);
    }

    if (redirect_mode == Request::RedirectMode::Manual) {
      // redirect: "manual" - return an opaque-redirect filtered response.
      // Set the response type to "opaqueredirect" and return the response as-is
      // (the 3xx status, headers, etc. are preserved).
    ...
    auto *response_handle = Response::maybe_response_handle(response_obj);
    MOZ_ASSERT(response_handle);
    auto headers_res = response_handle->headers();
    ...
    // Use jsurl to parse the location relative to the current request URL.
    ...
    // Copy the href string before freeing the URL object, as SpecSlice
    // points into the JSUrl's internal memory.
    std::string resolved_url_str(reinterpret_cast<const char *>(resolved_href.data),
                                 resolved_href.len);
    jsurl::free_jsurl(resolved_url);

    // 3. Determine the method for the redirected request.
    RootedString method_js(cx, Request::method(request));
    host_api::HostString method = core::encode(cx, method_js);
    if (!method.ptr) {
      return false;
    }

    std::string new_method(method.ptr.get(), method.len);

    // Per Fetch spec:
    // - 303: always change to GET, drop body
    // - 301, 302: change to GET if method is not GET or HEAD, drop body
    // - 307, 308: preserve method and body
    // Note: body preservation for 307/308 is not yet implemented; the redirected
    // request will not include a body. This matches common browser behavior for
    // most practical redirect scenarios.
    if (status == 303) {
      new_method = "GET";
    ...
    // 307, 308: preserve method (body replay not yet supported)

    // 4. Clone headers from the original request for the new request.
    ...
    // 5. Create and send the new outgoing request.
    host_api::HostString new_url = host_api::HostString(
        JS::UniqueChars(js_strdup(resolved_url_str.c_str())), resolved_url_str.size());
    ...
    // 6. Update the request object's URL to the new location.
    ...
    // 7. Queue a new ResponseFutureTask for the redirected request.
    auto task = js::MakeUnique<ResponseFutureTask>(request, pending_handle,
                                                   redirect_count_ + 1);
    api::Engine::queue_async_task(task.release());
  ...
  if (redirect_count_ > 0) {
    JS::SetReservedSlot(response_obj, std::to_underlying(Response::Slots::Redirected),
                        JS::TrueValue());
  }
